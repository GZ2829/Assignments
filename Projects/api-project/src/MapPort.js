    ...
    render() {
        ...
        return (
            <div className='ticker2' key={holder.id} onChange={this.handleChange}>
                ...
                <p style={{ color: this.props.totalNum[this.props.holder.id].value > 0.00 ? 'rgb(60, 255, 76)' : 'rgb(255, 85, 85)' }} className='toats'>{this.props.totalNum[this.props.holder.id].value > 0 ? `Current Earnings: $ ${Math.floor(this.props.totalNum[this.props.holder.id].value)}` : `Current Earnings: $ ${Math.floor(this.props.totalNum[this.props.holder.id].value)}`}</p>
